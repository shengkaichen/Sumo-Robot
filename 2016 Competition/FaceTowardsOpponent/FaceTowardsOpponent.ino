...
// A sensors reading must be greater than or equal to this threshold in order for the program to consider that sensor as seeing an object.
const uint8_t sensorThreshold = 1;

// The maximum speed to drive the motors while turning.  400 is full speed.
const uint16_t turnSpeedMax = 400;

// The minimum speed to drive the motors while turning.  400 is full speed.
const uint16_t turnSpeedMin = 100;

// The amount to decrease the motor speed by during each cycle when an object is seen.
const uint16_t deceleration = 10;

// The amount to increase the speed by during each cycle when an object is not seen.
const uint16_t acceleration = 10;

#define LEFT 0
#define RIGHT 1

// Stores the last indication from the sensors about what direction to turn to face the object.
// When no object is seen, this variable helps us make a good guess about which direction to turn.
bool senseDir = RIGHT;
...
void loop()
{
  ...
  if (objectSeen)
  {
    // An object is visible, so we will start decelerating in order to help the robot find the object without
